...
变更流程（严格执行，不可跳过）：
  ① 修改 .j2 模板文件
  ② 本地运行 Harness（MockLLM）：python3 scripts/run_harness_ci.py --mock
  ③ 准确率 ≥ 94% → 提交 PR
  ④ CI 自动触发 Harness 回归
  ⑤ 准确率 ≥ 94% → PR 可合并
...
教师审核后的必要动作（全部由服务层执行，不依赖教师操作）：
  ① grading_results.is_correct → 更新为 override_correct
  ...
  ⑦ 学生端结果更新（submissions.status 重新计算）
...
# ── 主力模型（锁定到具体版本，禁止使用 latest/chat 别名）──────
DEEPSEEK_MODEL_PARSER       = "deepseek-chat"      # 当前指向 DeepSeek-V3
...
# 4. 版本升级需要：重新运行全量 Harness → 确认准确率不下降
...
```
Phase 1（当前）：基础能力建立
  ✓ 四大工程范式全部落地（Prompt/Context/Harness/Loop）
  ✓ DeepSeek-V3 + Qianwen-Max 双模型支持
  ✓ Harness CI 基础体系（160+ 标注用例，MockLLM 离线运行）
  ✓ HITL 人工审核闭环（低置信 → 教师审核 → 学生通知）
