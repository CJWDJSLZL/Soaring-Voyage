...
| asyncpg 连接池大小 | **pool_size=25** | 基于 `CPU核数×2+磁盘数` 公式（8核服务器） |
...
| 轮询间隔（HITL 等待） | **2s / 120s 超时** | 平衡实时性与服务器压力 |
...
│   │   │
│   │   ├── db/
│   │   │   ├── pool.py                     ← asyncpg Pool 工厂（pool_size=25）
...
│   │       ├── hooks/
│   │       │   ├── useGradingPoller.ts     ← 轮询 Hook（2s/次，120s 超时）
│   │       │   ├── usePermission.ts        ← 角色权限检查 Hook
│   │       │   └── useNotification.ts      ← 待审核数量轮询（60s/次）
│   │       ├── api/
│   │       │   ├── client.ts               ← axios 实例 + token 刷新拦截器
...
- asyncpg 连接池（pool_size=25）+ TenantSession（SET LOCAL 注入）
...
**目标**：LangGraph 8节点管道端到端跑通，Harness MockLLM 准确率 ≥ 90%，CI 门禁工作。
...
- CI Harness 门禁：MockLLM 准确率 < 90% 阻断 PR 合并

**验收标准**：
```bash
pytest tests/integration/test_grading_pipeline.py  # 所有场景通过
python scripts/run_harness_ci.py --mock             # 准确率 ≥ 90%
# 提交 PR → CI Harness 自动触发 → 准确率 < 90% → CI 失败阻断合并
```
...
**Week 9 任务（学生端）**：
- 作业列表 + 逐题输入答案 + 提交按钮
- useGradingPoller（2s轮询，120s超时，HITL 等待状态）
...
async def create_pool(dsn: str) -> asyncpg.Pool:
    return await asyncpg.create_pool(
        dsn,
        min_size=5,
        max_size=25,                          # pool_size=25（8核 × 2 + 余量）
...
_DOMAIN_PADDING = (
    "【翱翔启航AI批改系统】本系统专为小学1-6年级数学作业批改设计，"
...
_PUBLIC_PATHS = frozenset({
    "/health", "/api/v1/auth/login", "/api/v1/auth/refresh", "/api/docs", "/openapi.json"
})
...
async def run_harness(cases_dir: str, use_mock: bool = True,
                      ...
                      accuracy_threshold: float = 0.90) -> HarnessResult:
...
interface AuthStore {
  user: User | null;  token: string | null;  refresh_token: string | null
  login:  (u: User, t: string, rt: string) => void
  logout: () => void
...
export const useAuthStore = create<AuthStore>()(
  persist(
    (set) => ({
      user: null, token: null, refresh_token: null,
      login:    (user, token, refresh_token) => set({ user, token, refresh_token }),
      logout:   () => set({ user: null, token: null, refresh_token: null }),
...
```typescript
// src/shared/hooks/useGradingPoller.ts
import { useEffect, useRef, useCallback } from 'react'
import { useSubmissionStore } from '../stores/submissionStore'
import { api } from '../api/client'

const POLL_MS = 2000       // 2秒轮询
const TIMEOUT_MS = 120000  // 120秒超时

export function useGradingPoller(submissionId: string | null) {
  ...
  const poll = useCallback(async () => {
    ...
    try {
      ...
      const allGraded = data.results?.every((r: any) => r.routed_to_human === false)
      if (data.status === 'graded' && allGraded) {
        setResult(data); setStatus('done')
      } else if (data.status === 'partial_human_review') {
        setStatus('hitl')   // 展示"老师正在批改"状态
        timerRef.current = setTimeout(poll, POLL_MS)
      } else {
        timerRef.current = setTimeout(poll, POLL_MS)
      }
    } catch {
      timerRef.current = setTimeout(poll, POLL_MS * 2)  // 网络错误时延长间隔
    }
  }, [submissionId])
...
// 响应拦截：401 时自动刷新 Token，刷新失败跳转登录页
...
test-backend:
  ...
  services:
    ...
    - name: redis:7-alpine
      alias: redis
  variables:
    DATABASE_URL: "postgresql+asyncpg://ci:ci@postgres/ci"
...
harness-mock:
  ...
  script:
    ...
    - |
      ...
      sys.exit(0 if r['accuracy'] >= 0.90 else 1)
      "
  artifacts:
...
DATABASE_URL=postgresql+asyncpg://user:pass@postgres:5432/aoxiang
...
- [ ] **16.** Harness MockLLM 最终验收运行：准确率 ≥ 90%（`python scripts/run_harness_ci.py --mock`）
...
| 缓存/限流 | Redis | 7-alpine | 滑动窗口限流；令牌桶 LLM 并发控制；HITL 队列 + Prompt 前缀缓存 |
