...
## 当前实现状态与未完成工作（2026-07-22）

> 本节记录当前停止开发时的真实状态，供后续接手时直接恢复上下文。除维护文档外，不应继续扩大代码改动范围。

### 已完成并合并到 `main` 的范围

| 范围 | 当前状态 | 验证证据 |
|------|----------|----------|
| 后端 MVP 内存实现 | 已完成 | 本地测试与 Harness 门禁通过 |
| PostgreSQL 第一阶段：认证与题库 | 已完成 | PR #2 合并，真实 PostgreSQL CI 通过 |
| PostgreSQL 第二阶段第一小步：作业创建/列表/详情 | 已完成 | PR #3 合并，`main` CI run `29879543891` 通过 |

截至本记录，`main` 最新已验证提交为：

```text
980fb6ad9f1c344cc8272eed14ab429bce4dc37c
```

PostgreSQL 模式下已经可用的业务纵切：

1. 登录、JWT 校验、`token_version` 失效机制；
2. 题库创建与查询；
3. 教师创建作业；
4. 教师/学生查询作业列表；
5. 教师/学生查看作业详情；
6. 上述路径均继续使用 asyncpg、显式事务、租户上下文和 RLS。

### 尚未完成的工作

| 优先级 | 未完成项 | 当前边界 | 完成标准 |
|--------|----------|----------|----------|
| P0 | 学生提交持久化 | PostgreSQL 模式下仍 fail-closed，不应读取内存提交数据 | `submissions`、`submission_answers` 事务写入；学生只能提交自己班级可见作业；重复提交/截止时间/权限规则通过 API 与 RLS 测试 |
| P0 | 自动批改结果持久化 | 当前批改闭环未落到 PostgreSQL | `grading_results` 保存每题结果、置信度、错误类型、反馈、agent trace；提交总状态与题目结果一致 |
| P0 | 提示重试持久化 | Hint 接口仍属于未迁移范围 | 每次 hint/重试记录 attempt；最多 3 级提示；错误题与历史结果可追溯 |
| P0 | 教师人工复核 HITL | 人工复核队列仍未迁移 | `human_review_queue` 入队、领取、通过/修改/拒绝全流程事务化；复核后清理完成条件字段并写审计 |
| P1 | SSE 实时事件 | PostgreSQL 模式下 SSE 仍不承诺完整事件源 | 提交后事件在事务提交后发布；支持心跳、重连、最终状态关闭；未上线前继续明确不可用 |
| P1 | 错题与知识点沉淀 | 错误历史与知识点统计未形成稳定 PostgreSQL 写入链路 | 写入 `student_error_history` 或后续等价表；教师端统计可从 PostgreSQL 重建 |
| P1 | 作业 PATCH 更新 | 创建/列表/详情已迁移，修改仍未迁移 | 教师仅能修改自己租户内作业；班级/题目关联更新事务化；已提交作业的可变字段规则明确 |
| P1 | 前端三端联调 | 当前主要是后端 API 与文档 | 学生端、教师端、管理员端对接真实 API，完成核心用户路径 |
| P1 | 生产部署到 `api.gazhiwo.shop` | DNS 已配置，但功能未完成，暂不部署 | 后端功能闭环完成后再配置反向代理、HTTPS、生产环境变量、健康检查与备份 |

### 下一次恢复开发的建议顺序

1. 先从 **学生提交持久化** 开始，不要先做 SSE 或前端，否则提交数据仍会断链。
2. 采用最小 TDD：每个纵切保留 1-2 个关键 API/Repository 测试即可，避免测试数量膨胀；但必须包含真实 PostgreSQL CI 验证。
3. 学生提交完成后再接入自动批改结果持久化，并确保提交总状态、逐题结果和 HITL 入队条件在一个一致的数据模型下闭合。
4. HITL 复核完成前，教师端统计、SSE 最终态和错题历史都只能作为后续工作，不应宣称生产可用。
5. 所有已迁移路径继续走 PostgreSQL；所有未迁移路径继续明确 `503 / 5003`，禁止静默回退到内存实现。

### 继续开发时必须保留的质量门槛

- 后端 pytest 覆盖率不得低于 `80%`；
- Harness 固定用例不少于 `180`，准确率不得低于 `94%`；
- Ruff、Ruff Format、Mypy、Bandit 必须通过；
- 真实 PostgreSQL CI 必须以非超级运行时角色验证 RLS；
- 不得修改已发布的 `backend/migrations/001_initial_schema.sql`，新增数据库变更必须使用后续 migration；
- 不得提交 API Key、数据库连接串、私钥或任何真实凭据。
