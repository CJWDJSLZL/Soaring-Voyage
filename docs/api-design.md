...
- `submission_status`：仅学生角色返回，`not_submitted | submitted | partial_human_review`
...
```json
{
  ...
  "answers": [
    ...
    {"problem_id": "uuid3", "answer_text": "三分之二"}
  ]
}
```
...
- `answers`：数组长度 1-50；`answer_text` 最长 500 字符，去除首尾空格后不能为空（空字符串允许提交，系统判错）
  ...
  "data": {
    ...
    "results": [
      ...
      },
      {
        "problem_id": "uuid3",
        "sequence": 3,
        "problem_text": "把3÷4写成分数是___",
        "student_answer": "三分之二",
        "is_correct": null,
        "confidence_score": 0.71,
        "feedback_text": "老师正在帮你仔细批改这道题，稍后会通知你结果。",
        "encouragement": null,
        "next_hint": null,
        "error_type": null,
        "hint_level": 0,
...
```
POST /api/v1/problems/bulk-import → 202 { "import_job_id": "uuid" }
GET  /api/v1/ops/jobs/{job_id}    → { "status": "running|completed|failed", "progress": 0.6 }
```
